...
final class WDPELoginPacketHandler extends PacketHandler {
	...
	const PROTOCOL_ALIASING = [
		818 => 819, // 1.21.90 -> 1.21.93 (No changes)
	];
	...
	public function handleLogin(LoginPacket $packet): bool {
		...
		try {
			$skin = (new LegacySkinAdapter())->fromSkinData(self::fromClientData($clientData));
		...
		if ($this->server->getNetwork()->getConnectionCount() > $this->server->getMaxPlayers()) {
			$ev->setKickFlag(PlayerPreLoginEvent::KICK_FLAG_SERVER_FULL, KnownTranslationKeys::DISCONNECTIONSCREEN_SERVERFULL);
		}
		if (!$this->server->isWhitelisted($playerInfo->getUsername())) {
			$ev->setKickFlag(PlayerPreLoginEvent::KICK_FLAG_SERVER_WHITELISTED, "Server is whitelisted");
		}
		if ($this->server->getNameBans()->isBanned($playerInfo->getUsername()) or $this->server->getIPBans()->isBanned($this->session->getIp())) {
			$ev->setKickFlag(PlayerPreLoginEvent::KICK_FLAG_BANNED, "You are banned");
		}

		$ev->call();
		if (!$ev->isAllowed()) {
			$this->session->disconnect($ev->getFinalDisconnectReason());
			return true;
		}
	...
	protected function parseJwtChain(string $authInfoJson): JwtChain {
		$jwtChainData = json_decode(json_decode($authInfoJson, true)["Certificate"], true);
		if (!is_array($jwtChainData) || !isset($jwtChainData["chain"])) {
			throw new InvalidArgumentException("Malformed JWT chain data");
		}

		$jwtChain = new JwtChain;
		$jwtChain->chain = $jwtChainData["chain"];
		return $jwtChain;
	}
	...
	protected function processLogin(LoginPacket $packet, bool $authRequired): void {
		$this->server->getAsyncPool()->submitTask(new ProcessLoginTask($this->parseJwtChain($packet->authInfoJson)->chain, $packet->clientDataJwt, $authRequired, $this->authCallback));
		$this->session->setHandler(null); //drop packets received during login verification
	}

	protected function isCompatibleProtocol(int $protocolVersion): bool {
		foreach (self::PROTOCOL_ALIASING as $min => $max) {
