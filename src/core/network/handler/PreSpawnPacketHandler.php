...
use pocketmine\network\mcpe\protocol\MovementPredictionSyncPacket;
	...
	public function setUp(): void {
		...
		try {
			...
			$levelSettings->experiments = new Experiments([
				"data_driven_items" => true // allows for custom block textures to be registered client-side
			], true);
			...
			$this->session->sendDataPacket(StartGamePacket::create(
				...
				BlockRegistry::$blockPaletteEntries, // custom block textures
				0
			));

			$this->session->getLogger()->debug("Sending items.");
			$this->session->sendDataPacket(ItemRegistryPacket::create(array_merge($typeConverter->getItemTypeDictionary()->getEntries(), array_values(ItemRegistry::$itemTypeEntries))));
