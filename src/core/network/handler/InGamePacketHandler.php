...
use core\utils\Utils as UtilsUtils;
...
class InGamePacketHandler extends PacketHandler {
	...
	public function handlePlayerAuthInput(PlayerAuthInputPacket $packet): bool {
		...
		if ($inputFlags->get(PlayerAuthInputFlags::MISSED_SWING)) {
			$this->player->getAntiCheatSession()?->clicked();
		}
	...
	private function executeInventoryTransaction(InventoryTransaction $transaction, int $requestId): bool {
		...
		if ($this->player->isAFK()) return false;
	...
	private function handleNormalTransaction(NormalTransactionData $data, int $itemStackRequestId): bool {
		...
		if (
			...
			($serverItemStack->getBlockRuntimeId() !== $clientItemStack->getBlockRuntimeId() && $serverItemStack->getBlockRuntimeId() !== 0) // for custom blocks, runtime doesn't exist server-side
	...
	private function handleUseItemOnEntityTransaction(UseItemOnEntityTransactionData $data): bool {
		...
		if ($target === null || $this->player->isAFK()) {
			return false;
		}
		...
		switch ($data->getActionType()) {
			...
			case UseItemOnEntityTransactionData::ACTION_ATTACK:
				$this->player->getAntiCheatSession()?->clicked($target);
	...
	private function handlePlayerActionFromData(int $action, BlockPosition $blockPosition, int $face): bool {
		...
		switch ($action) {
			...
			case PlayerAction::STOP_ITEM_USE_ON:
				...
				break;
			default:
				UtilsUtils::dumpVals("Unknown player action type: " . $action);
	...
	public function handleLevelSoundEvent(LevelSoundEventPacket $packet): bool {
		return $this->handleClickSound($packet);
	}
	...
	public function handleClickSound(LevelSoundEventPacket $packet): bool {
		if (!isset($packet->sound)) return false;
		if (in_array($packet->sound, [LevelSoundEvent::ATTACK, LevelSoundEvent::ATTACK_NODAMAGE, LevelSoundEvent::ATTACK_STRONG])) $this->session->getPlayer()?->getAntiCheatSession()?->clicked();
		return true;
	}
}
