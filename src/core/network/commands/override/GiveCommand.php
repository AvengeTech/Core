...
class GiveCommand extends CoreCommand{
	...
	protected function fetchPermittedPlayerTarget(CommandSender $sender, ?string $target, string $selfPermission, string $otherPermission): ?Player {
		...
		/** @var AtPlayer $player */
		if (
			($player === $sender && $player->getRankHierarchy() >= Rank::HIERARCHY_SR_MOD) ||
			($player !== $sender && $player->getRankHierarchy() >= Rank::HIERARCHY_HEAD_MOD)
		) {
			return $player;
		}
		return null;
