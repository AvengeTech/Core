...
class ViewMuteUi extends SimpleForm {
	...
	public function __construct(AtPlayer $player, public MuteEntry $entry, public array $mutes, public User $user) {
		...
		parent::__construct(
			...
			"Moderator: " . $byUser->getGamertag() . PHP_EOL .
				...
				PHP_EOL .
				"Status: " . ($this->entry->isRevoked() ? "Revoked" : ($this->entry->isMuted() ? "Active" : "Expired")) . PHP_EOL . PHP_EOL .
	...
	public function handle($response, AtPlayer $player) {
		if ($response == -1 + $this->offset) return $player->showModal(new ModifyMuteUi($player, $this->entry, $this->mutes, $this->user));
		elseif ($response == 0 + $this->offset) {
			if ($this->entry->isRevoked()) return $player->showModal(new MutesListUi($player, $this->mutes, $this->user));
			elseif ($player->getRankHierarchy() >= Rank::HIERARCHY_MOD) return $player->showModal(new ConfirmRevokeUi($this->entry, $this->mutes, $this->user));
		}
		$player->showModal(new MutesListUi($player, $this->mutes, $this->user));
	}
}
