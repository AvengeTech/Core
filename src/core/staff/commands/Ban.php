	...
	public function handle(CommandSender $sender, string $commandLabel, array $args): void {
		...
		if (count($args) == 0) {
			if (!$sender instanceof Player) {
				Core::getInstance()->getStaff()->ban($username, "sn3akrr", "Chargeback");
				$sender->sendMessage(TextFormat::GN . $username . " has been banned permanently for chargeback!");
				return;
			}
		...
		$check = function (User $senderUser, User $user) use ($sender, $until, $reason): void {
			Core::getInstance()->getStaff()->loadBans($user, function (BanManager $bm) use ($user, $sender, $senderUser, $until, $reason): void {
				if (!$user->valid()) {
					$sender->sendMessage(TextFormat::RI . "Player never seen!");
					return;
				}
				if ($bm->isBanned()) {
					$sender->sendMessage(TextFormat::RI . "This player is already banned!");
					return;
				}
				Core::getInstance()->getStaff()->ban($user, $senderUser, $reason, $until);
				$sender->sendMessage(TextFormat::GI . $user->getGamertag() . " has been banned!");
			});
		};
