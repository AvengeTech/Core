...
use core\gadgets\entity\animation\FireworkParticleAnimation;
...
class Firework extends Entity {
	...
	public function __construct(Location $location, FireworkItem $fireworks, ?int $lifeTime = null) {
	...
	public function playSounds(): void {
		...
		foreach ($explosionsTag->getValue() as $info) {
			if ($info instanceof CompoundTag) {
				if ($info->getByte("FireworkType", 0) === FireworkItem::TYPE_HUGE_SPHERE) {
